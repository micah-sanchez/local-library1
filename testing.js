...
//   //need to access existing books in borrowed result
//   //if book is in the result, add borrowed data to the book
//   //else push the entire book 
//           if (borrowedResult.includes(book.id)) {
//             console.log(true);
//           } else {
//             borrowedResult.push({
//               id: book.id, 
...
const books = [
  ...
  function getMostCommonGenres(books) {
    ...
    for (let i = 0; i < books.length; i++) {
      const book = books[i]
      const genres = Object.keys(genreArray)
      if (genreArray.length === 0) {
        genreArray.push({genre: book.genre, count: 1})
      } 
      // else if (Object.keys(genreArray.genre).includes(book.genre)) {
      //   genreArray[book].count += 1
      // } 
      else {
        genreArray[book].count += 1
      }
